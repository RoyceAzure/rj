...
func TestLoggerProduce(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			LogTopicName = fmt.Sprintf("tMsgs-%d_tps-%d_bsize-%d_ival-%dms",
				testCase.testMsgs,
				int((float64(testCase.loggerNum*testCase.each_publish_num) * 1000 / float64(testCase.each_publish_duration.Milliseconds()))),
				cfg.BatchSize,
				cfg.CommitInterval.Milliseconds(),
			)
