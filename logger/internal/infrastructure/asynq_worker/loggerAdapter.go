...
func Print(level zerolog.Level, args ...interface{}) {
	producer.Logger.WithLevel(level).Msg(fmt.Sprint(args...))
}

func (logger loggerAdapter) Debug(args ...interface{}) {
	producer.Logger.WithLevel(zerolog.DebugLevel).Msg(fmt.Sprint(args...))
}

// Info logs a message at Info level.
func (logger loggerAdapter) Info(args ...interface{}) {
	producer.Logger.WithLevel(zerolog.InfoLevel).Msg(fmt.Sprint(args...))
}

// Warn logs a message at Warning level.
func (logger loggerAdapter) Warn(args ...interface{}) {
	producer.Logger.WithLevel(zerolog.WarnLevel).Msg(fmt.Sprint(args...))
}

// Error logs a message at Error level.
func (logger loggerAdapter) Error(args ...interface{}) {
	producer.Logger.WithLevel(zerolog.ErrorLevel).Msg(fmt.Sprint(args...))
}

// Fatal logs a message at Fatal level
// and process will exit with status set to 1.
func (logger loggerAdapter) Fatal(args ...interface{}) {
	producer.Logger.WithLevel(zerolog.FatalLevel).Msg(fmt.Sprint(args...))
}
